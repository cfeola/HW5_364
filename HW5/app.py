...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/cfeolaHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    new_priority = FloatField("What is the new priority of the todo list item", validators=[Required()])
    submit = SubmitField("Update")
...
def delete(lst):
    todo_list = TodoList.query.filter_by(title = lst).first()
